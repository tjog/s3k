...
int main(void)
{
	...
	err = s3k_path_derive(path_idx, "app", tmp_idx, 4096, PATH_READ | PATH_WRITE);
	if (err) {
	...
	// Create dir
	alt_puts("Create dir:");
	err = s3k_path_derive(path_idx, NULL, tmp_idx, 512, PATH_READ | PATH_WRITE);
	if (err) {
		alt_printf(PROCESS_NAME ": error: s3k_cap_derive returned error %d\n", err);
		return -1;
	}
	...
	// Write file
	alt_puts("Write file:");
	err = s3k_path_derive(path_idx, "test.txt", tmp_idx, 1024, FILE | PATH_WRITE);
	if (err) {
		alt_printf(PROCESS_NAME ": error: s3k_cap_derive returned error %d\n", err);
		return -1;
	}
	...
	uint32_t bw = 0;
	err = s3k_write_file(tmp_idx, 0, b, sizeof(b), &bw);
	if (err) {
	...
	alt_printf(PROCESS_NAME ": expecting %d less create quota\n", sizeof(partial_overlap) - 4);
	dump_caps_range(PROCESS_NAME, path_idx, tmp_idx);
	err = s3k_cap_delete(tmp_idx);
	if (err) {
		alt_printf(PROCESS_NAME ": error: s3k_cap_delete returned error %d\n", err);
	...
	alt_printf("Expecting cap %d to have grown\n", tmp_idx);
	dump_caps_range(PROCESS_NAME, path_idx, tmp_idx);
	err = s3k_cap_delete(tmp_idx);
	if (err) {
		alt_printf(PROCESS_NAME ": error: s3k_cap_delete returned error %d\n", err);
