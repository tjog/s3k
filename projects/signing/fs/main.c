...
static inline bool check_pmp(s3k_cidx_t pmp_cidx, uint8_t *buf, uint32_t len, s3k_rwx_t rwx_mask)
{
	...
	if (!cap.pmp.used) {
		...
		if (err == S3K_ERR_DST_OCCUPIED) {
			size_t loaded_client = 0;
			for (; loaded_client < MAX_CLIENTS; loaded_client++) {
				if (!clients[loaded_client].active) {
					continue;
				}
				s3k_cidx_t loaded_client_pmp_cidx
				    = clients[loaded_client].pmp_cap_idx;
				if (loaded_client_pmp_cidx == pmp_cidx)
					continue;
				s3k_cap_t loaded_client_pmp_cap;
				if (err
				    = s3k_cap_read(loaded_client_pmp_cidx, &loaded_client_pmp_cap))
					return false;
				if (loaded_client_pmp_cap.pmp.used) {
					err = s3k_pmp_unload(loaded_client_pmp_cidx);
					if (err) {
						alt_printf(
						    PROCESS_NAME
						    ": unloading PMP slot to load another clients PMP capability failed with error: %d\n",
						    err);
						return false;
					}
			...
			if (err) {
				alt_printf(
				    PROCESS_NAME
				    ": loading client's PMP slot after unloading the existing PMP capability failed with error: %d\n",
				    err);
				return false;
			}
		} else if (err != S3K_SUCCESS) {
			alt_printf(PROCESS_NAME
				   ": loading client's PMP slot failed with error: %d\n",
				   err);
			return false;
		}
		s3k_sync_mem();
...
s3k_msg_t do_fs_client_init(s3k_reply_t recv_msg, s3k_cidx_t receive_cidx)
{
	...
	if (recv_msg.cap.type != S3K_CAPTY_PMP) {
		...
		alt_printf(PROCESS_NAME
			   ": CLIENT INITIALIZED (%d), socket_tag=%d, pmp_cap_idx=%d, active=%d\n",
...
s3k_msg_t do_fs_client_finalize(s3k_reply_t recv_msg)
{
	...
	// Unloads the PMP in case it is loaded
...
s3k_msg_t do_fs_read_dir(s3k_reply_t recv_msg, s3k_cidx_t recv_cidx, client_t *c)
{
	...
	s3k_err_t err = s3k_path_read(recv_cidx, buf, sizeof(buf));
	if (err) {
		alt_printf(PROCESS_NAME ": error: s3k_path_read returned %d\n", err);
		response.data[0] = FS_ERR_INVALID_CAPABILITY;
		return response;
	}
	...
	if (!check_pmp(c->pmp_cap_idx, (uint8_t *)buf_ptr, buf_len, S3K_MEM_W)) {
		response.data[0] = FS_ERR_INVALID_MEMORY;
		return response;
	}
