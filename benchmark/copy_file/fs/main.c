...
fs_err_t write_file(char *path, uint32_t offset, uint8_t *buf,
		    uint32_t buf_size, uint32_t *bytes_written)
{
	FIL Fil; /* File object needed for each open file */
	FRESULT fr;
	fs_err_t err = FS_SUCCESS;
...
static inline bool check_pmp(s3k_cidx_t pmp_cidx, uint8_t *buf, uint32_t len,
			     s3k_rwx_t rwx_mask)
{
	s3k_cap_t cap;
	...
	if (!is_range_subset(pmp_base, pmp_base + pmp_len, (uint64_t)buf,
			     (uint64_t)buf + len))
		return false;
	if (!cap.pmp.used) {
...
fs_err_t setup_pmp_from_mem_cap(s3k_cidx_t mem_cap_idx, s3k_cidx_t pmp_cap_idx,
				s3k_pmp_slot_t pmp_slot, s3k_napot_t napot_addr,
				s3k_rwx_t rwx)
{
	fs_err_t err = FS_SUCCESS;
...
s3k_msg_t do_fs_client_init(s3k_reply_t recv_msg, s3k_cidx_t receive_cidx)
{
	...
	if (recv_msg.cap.type != S3K_CAPTY_PMP) {
		...
		alt_printf(
		    ...
		    clients[client_num].pmp_cap_idx,
		    clients[client_num].active);
	}
	return response;
}
...
s3k_msg_t do_fs_create_dir(s3k_reply_t recv_msg, s3k_cidx_t recv_cidx)
{
	...
	alt_printf(
	    PROCESS_NAME
	    ": CLIENT (tag=%d) create_dir(path=%s, ensure_create=%d) = %d\n",
	    recv_msg.tag, buf, ensure_create, response.data[0]);
	return response;
}

s3k_msg_t do_fs_read_file(s3k_reply_t recv_msg, s3k_cidx_t recv_cidx,
			  client_t *c)
{
	s3k_msg_t response = {0};
	response.send_cap = false;
	...
	fs_err_t fserr
	    = read_file(buf, file_offset, buf_ptr, buf_len, &bytes_read);
...
s3k_msg_t do_fs_write_file(s3k_reply_t recv_msg, s3k_cidx_t recv_cidx,
			   client_t *c)
{
	s3k_msg_t response = {0};
	response.send_cap = false;
	...
	alt_printf("calling write_file(%s, %d, 0x%x, %d)\n", buf, file_offset,
		   buf_ptr, buf_len);
	fs_err_t fserr
	    = write_file(buf, file_offset, buf_ptr, buf_len, &bytes_written);
	response.data[0] = fserr;
	if (!fserr) {
...
s3k_msg_t do_fs_read_dir(s3k_reply_t recv_msg, s3k_cidx_t recv_cidx,
			 client_t *c)
{
	s3k_msg_t response = {0};
	response.send_cap = false;
	...
	s3k_dir_entry_info_t *buf_ptr
	    = (s3k_dir_entry_info_t *)recv_msg.data[2];
	uint64_t buf_len = sizeof(s3k_dir_entry_info_t);
	if (!check_pmp(c->pmp_cap_idx, (uint8_t *)buf_ptr, buf_len,
		       S3K_MEM_R)) {
		response.data[0] = FS_ERR_INVALID_MEMORY;
		return response;
	}
...
s3k_msg_t do_fs_delete_entry(s3k_reply_t recv_msg, s3k_cidx_t recv_cidx)
{
	...
	alt_printf(PROCESS_NAME ": CLIENT (tag=%d) path_delete(path=%s) = %d\n",
		   recv_msg.tag, buf, response.data[0]);
	return response;
}
	...
	*/
	...
	if (server_cidx == S3K_CAP_CNT) {
		alt_printf(PROCESS_NAME
			   ": error: could not find file server's socket\n");
		return -1;
	}
	...
	msg_received:
		if (recv_msg.err) {
			alt_printf(PROCESS_NAME
				   ": error: s3k_sock_recv returned error %d\n",
				   recv_msg.err);
			continue;
		}
		alt_printf(PROCESS_NAME
			   ": received from tag=%d, data=[%d, %d, %d, %d]",
			   recv_msg.tag, recv_msg.data[0], recv_msg.data[1],
		...
		bool should_receive_cap
		    = fs_server_should_receive_cap(recv_msg.data[0]);
		...
		case fs_create_dir: {
			if (!ensure_client_connected(recv_msg.tag))
				...
				response
				    = do_fs_create_dir(recv_msg, recv_cidx);
		} break;
		case fs_read_file: {
			size_t client_idx = get_client(recv_msg.tag);
			if (client_idx == MAX_CLIENTS)
				...
				response = do_fs_read_file(
				    recv_msg, recv_cidx, &clients[client_idx]);
		} break;
		case fs_write_file: {
			size_t client_idx = get_client(recv_msg.tag);
			if (client_idx == MAX_CLIENTS)
		...
		case fs_delete_entry: {
			if (!ensure_client_connected(recv_msg.tag))
				...
				response
				    = do_fs_delete_entry(recv_msg, recv_cidx);
		} break;
		case fs_read_dir: {
			size_t client_idx = get_client(recv_msg.tag);
			if (client_idx == MAX_CLIENTS)
				...
				response = do_fs_read_dir(recv_msg, recv_cidx,
							  &clients[client_idx]);
		} break;
		default: {
			response.send_cap = false;
