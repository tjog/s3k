...
uint8_t sample_buf[4 * 1024] __attribute__((aligned(4 * 1024))); // 32KiB
...
void cleanup()
{
	...
	s3k_reply_t reply = send_fs_delete_entry(client_cidx, S3K_CAP_CNT - 2);
	ASSERT(reply.err);
	ASSERT(reply.data[0]);
	// Derive a throwaway capability to send
	ASSERT(s3k_path_derive(S3K_CAP_CNT - 1, NULL, S3K_CAP_CNT - 2,
	...
	reply = send_fs_write_file(client_cidx, S3K_CAP_CNT - 2, 0, sample_buf,
				   sizeof(sample_buf));
	ASSERT(reply.err);
	ASSERT(reply.data[0]);
	ASSERT(!(reply.data[1] == sizeof(sample_buf)));
}
